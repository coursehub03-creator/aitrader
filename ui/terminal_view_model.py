...
def prepare_recommendation_panel(recommendation: FinalRecommendation | None) -> dict[str, Any]:
    ...
    return {
        ...
        "fields": [
            ...
            ("historical_score", "n/a" if recommendation.historical_score is None else f"{recommendation.historical_score:.2f}"),
            ("recent_score", "n/a" if recommendation.recent_score is None else f"{recommendation.recent_score:.2f}"),
            ("combined_score", "n/a" if recommendation.combined_score is None else f"{recommendation.combined_score:.2f}"),
